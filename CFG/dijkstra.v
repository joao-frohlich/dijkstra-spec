...
Section Dijkstra.
  ...
  Proof.
    intros.
    intros v x ?.
    destruct (vertex_eq_dec v u).
    - (* u is the vertex we are relaxing *)
      subst v.
      unfold sane in H.
      apply H.
      induction Q; simpl in H0; try lia.
      destruct Q.
      + compute in H1.
        rewrite H1 in H2.
        compute in H2.
        assert (Q' = []).
        { rewrite H2. destruct vertex_eq_dec; try intuition. }
        clear H2.
        subst Q'.
        compute in H3.
        subst N.
        ...
        subst d'.
        apply H5.
      + assert (length (v :: Q) > 0) by (simpl; lia).
        apply IHQ with H6.
        * destruct (vertex_eq_dec u a); simpl.
          -- (* if u = a, then we need to reach in a contradiction *)
             admit.
          -- (* if u <> a, then we need to take a out of take_shortest and remove *)
             admit.
        * admit.
    - (* u isn't the vertex we are relaxing *)
      unfold sane in H.
    admit.
  Admitted.
